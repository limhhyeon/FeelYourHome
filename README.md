...
| `서버 재시작 시 MQTT 구독 복원 문제` | 서버가 재시작된 후, MQTT에서 구독한 토픽들이 사라지고 다시 구독을 등록해야 하는 문제 발생. 특히, 상품 등록 시에만 구독을 하도록 설정했지만, 서버가 꺼졌다 켜지면 모든 구독 정보가 사라져 다시 등록해야 함. | 서버가 재시작될 때 활성화된 상품에 대해서만 구독을 다시 하도록 설정. 불필요한 구독을 피하기 위해 데이터베이스에서 활성화된 상품만 구독하도록 필터링하고, 페이지네이션을 통해 한 번에 너무 많은 데이터를 처리하지 않도록 수정. 
1. **MQTT 연결 재시도 로직 추가**: MQTT 연결이 제대로 되지 않으면 최대 5번까지 재시도하며 연결을 시도. 연결이 실패하면 로그를 찍고 구독 초기화 실패 처리. 
2. **활성화된 상품만 구독 복원**: 서버가 재시작될 때 데이터베이스에서 활성화된 상품들만 조회하여 구독을 다시 등록. 페이지네이션을 사용해 한 번에 너무 많은 데이터를 처리하지 않도록 함. `userProductRepository.findActiveMqttTopicsByActive(PageRequest.of(page, size))`를 사용해 활성화된 토픽들을 페이지 단위로 조회. |
